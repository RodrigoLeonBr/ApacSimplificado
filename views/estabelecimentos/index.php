...
<div class="mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Estabelecimentos de Saúde</h1>
    ...
    <div class="mb-4">
        ...
        <div class="relative">
            <input 
                type="text" 
                x-model="searchTerm"
                @input.debounce.500ms="buscarEstabelecimentos()"
                placeholder="Digite CNES, CNPJ ou Razão Social..."
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">CNES</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <template x-if="!loading && estabelecimentos.length === 0">
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                            </svg>
                            <p class="mt-2 text-lg font-medium">Nenhum estabelecimento encontrado</p>
                            <p class="text-sm text-gray-400">Tente ajustar os termos da busca</p>
                        </td>
                    </tr>
                </template>
                <template x-if="!loading && estabelecimentos.length > 0">
                    <template x-for="est in estabelecimentos" :key="est.id">
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" x-text="est.cnes"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="formatarCnpj(est.cnpj)"></td>
                            <td class="px-6 py-4 text-sm text-gray-900" x-text="est.razao_social"></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="est.municipio || '-'"></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span 
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" 
                                    :class="est.status === 'ativo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'"
                                    x-text="est.status === 'ativo' ? 'Ativo' : 'Inativo'">
                                </span>
                            </td>
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm text-gray-700">
                    Mostrando 
                    <span class="font-medium" x-text="estabelecimentos.length > 0 ? ((currentPage - 1) * 10 + 1) : 0"></span>
                    a 
                    <span class="font-medium" x-text="Math.min(currentPage * 10, totalEstabelecimentos)"></span>
                    de 
                    <span class="font-medium" x-text="totalEstabelecimentos"></span>
                    estabelecimentos
                </p>
            </div>
            <div>
