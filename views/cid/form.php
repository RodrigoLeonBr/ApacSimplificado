...
$title = ($cid ? 'Editar' : 'Novo') . ' CID - Sistema APAC';
$isEdit = $cid !== null;
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-800"><?= $isEdit ? 'Editar CID' : 'Novo CID' ?></h1>
            <p class="text-gray-600"><?= $isEdit ? 'Atualize os dados do CID' : 'Cadastre um novo CID no sistema' ?></p>
        </div>
...
<form action="<?= $action ?>" method="<?= $method ?>" class="bg-white rounded-lg shadow-md p-6">
    ...
    <div class="mb-6">
        ...
        <input 
            type="text" 
            name="codigo" 
            value="<?= htmlspecialchars($old['codigo'] ?? $cid['codigo'] ?? '') ?>"
            required
            maxlength="10"
    ...
    <div class="mb-6">
        ...
        ><?= htmlspecialchars($old['descricao'] ?? $cid['descricao'] ?? '') ?></textarea>
    ...
    <div class="flex items-center justify-end gap-4 pt-4 border-t">
        <a href="<?= UrlHelper::url('/cid') ?>" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition">
            Cancelar
        </a>
        <button 
            type="submit" 
