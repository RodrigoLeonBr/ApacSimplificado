...
class Log
{
    ...
    {
        $sql = "SELECT * FROM {$this->table} WHERE usuario_id = :usuario_id ORDER BY id DESC LIMIT :limit";
        return $this->db->fetchAll($sql, ['usuario_id' => $usuarioId, 'limit' => $limit]);
    }
    
    public function create($data)
    {
        $sql = "INSERT INTO {$this->table} (acao, usuario_id, tabela_afetada, registro_id, detalhes) 
                VALUES (:acao, :usuario_id, :tabela_afetada, :registro_id, :detalhes)";
